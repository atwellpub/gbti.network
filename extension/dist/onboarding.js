...
`;
  ...
  function prLifecycle(pull = {}, status2 = null) {
    ...
    const descIsReason = phase !== "rejected" || status2?.state === "failure" || status2?.state === "error";
  ...
  function filterByStatus(items, status2 = "all") {
    const list = Array.isArray(items) ? items : [];
    if (status2 === "published") return list.filter((x) => x?.status === "published");
    if (status2 === "draft") return list.filter((x) => x?.status !== "published");
    return list;
  }
  ...
  function editorStatus({ staged = false, status: status2 = "", publishedAt = "" } = {}) {
    if (staged) return { label: "Staged draft", tone: "staged", publishedLabel: "" };
    if (String(status2).toLowerCase() === "published") {
...
`;
  ...
  var GbtiContentEditor = class extends GbtiElement {
    ...
    async render() {
      ...
      const status2 = editorStatus({ staged: this.staged, status: p.status, publishedAt: p.publishedAt });
      const fmtD = (d) => {
        if (!d) return "";
        const t = new Date(d);
        return Number.isNaN(t.getTime()) ? "" : t.toISOString().slice(0, 10);
           ...
           <aside class="rail">
             <section class="rcard rcard-status">
               ...
               <div class="rcard-b">
                 ...
                 ${status2.publishedLabel ? `<div class="strow"><span class="sk">Published</span><span class="sv mono">${esc(status2.publishedLabel)}</span></div>` : ""}
...
`;
  var GbtiCategoriesWorkspace = class extends GbtiElement {
    ...
    _discordHtml(key) {
      ...
      const status2 = pendingOp ? "Pending house PR" : mapped ? "Synced (in the git map)" : "No channel linked";
      ...
      <div class="dcard">
        <div class="row1"><span class="sav">G</span><div><b>GBTI Network</b><div class="st">${esc(status2)}</div></div></div>
        <div class="pickrow">
...
`;
  var GbtiCouponManager = class extends GbtiElement {
    ...
    async load() {
      ...
      try {
        this._coupons = (await this.client.couponPool())?.coupons || [];
      } catch {
        this._coupons = [];
        this._msg = "Could not load the coupon registry.";
      }
      try {
...
`;
  ...
  var GbtiSyndicationTracker = class extends GbtiElement {
    ...
    render() {
      if (!this.client) {
        this.set(this.css(CSS16) + `<p class="muted">Open in the GBTI client (admin) to view the publishing activity.</p>`);
        return;
      }
      if (this._err) {
        this.set(this.css(CSS16) + `<p class="msg err">${esc(this._msg)}</p><button class="cancel" data-reload type="button" style="color:var(--accent)">Retry</button>`);
      ...
      if (!this._data) {
        ...
        this.set(this.css(CSS16) + `<p class="muted">Loading the publishing activity...</p>`);
        return;
      }
    ...
    _channels(perChannel) {
      ...
      return Object.entries(perChannel).map(([name, r]) => {
        ...
        return r?.url ? `<a class="ch ${esc(status2)}" href="${esc(r.url)}" target="_blank" rel="noopener">${esc(name)}</a>` : `<span class="ch ${esc(status2)}">${esc(name)}: ${esc(status2)}</span>`;
      }).join("");
    }
    async _approve(id) {
      if (!id) return;
...
`;
  ...
  var GbtiChannelMapManager = class extends GbtiElement {
    ...
    render() {
      if (!this.client) {
        this.set(this.css(CSS17) + `<p class="muted">Open in the GBTI client (superadmin) to manage the channels.</p>`);
        return;
      }
      if (!this._loaded) {
        if (!this._loading) {
    ...
    _wire() {
      ...
      this.$$("[data-tt]").forEach((t) => t.addEventListener("click", () => {
        this._termTab = t.dataset.tt;
        this._termFilter = "";
        this.render();
      }));
      ...
      const addTerm = () => {
        const input2 = this.$("[data-term-input]");
        const term = (input2?.value || "").trim();
        if (!term) {
...
`;
  var GbtiSubscribe = class extends GbtiElement {
    ...
    render() {
      ...
      this.set(
        this.css(CSS20) + `<button class="btn ${onCls}" type="button" aria-pressed="${following}" ${username ? "" : "disabled"} aria-label="${label}">${mega}<span class="t">${label}</span></button>`
      );
...
`;
  var GbtiUpvote = class extends GbtiElement {
    render() {
      ...
      this.set(
        this.css(CSS23) + `<button class="pill ${this._voted ? "on" : ""}" type="button" aria-pressed="${this._voted}" aria-label="${label}" title="${label}">${arrow(this._voted)}<span class="c">${c}</span></button>`
      );
      this.on(".pill", "click", () => this._onClick(targetType, targetSlug));
    }
    _onClick(targetType, targetSlug) {
...
`;
  ...
  var GbtiSharesFeed = class extends GbtiElement {
    ...
    async reload(quiet = false) {
      if (!this.client) {
        if (!quiet) this.set(this.css(CSS24) + `<p class="muted">Open in the GBTI client to read Shares.</p>`);
        return;
      }
      if (!quiet) this.set(this.css(CSS24) + `<p class="muted">Loading the co-op stream…</p>`);
      let membership = "unknown";
      try {
        const st = await this.client.status();
        membership = st?.membership ?? "unknown";
    ...
    _renderReading(share) {
      ...
      const link = share.url ? `<a class="link" href="${esc(utmLink(share.url, { ...UTM, utm_medium: "extension", utm_campaign: "shares" }))}" target="_blank" rel="noopener nofollow">${embedUrl(share.url) ? "Watch video" : "Read article"} on ${esc(hostOf2(share.url))}</a>` : "";
      ...
      this.set(this.css(CSS24) + `<div class="rtop"><button class="back" type="button" data-back>&larr; Back to the stream</button>${mod}</div>
    ...
    _splash() {
      this.set(this.css(CSS24) + `<div class="splash"><div class="lock">🔒</div><h3>Your access is locked</h3>
...
`;
  var GbtiShares = class extends GbtiElement {
    render() {
      this.set(this.css(CSS25) + `<div class="stack">
...
`;
  var GbtiLockGate = class extends GbtiElement {
    ...
    async _check() {
      this.set(this.css(CSS26) + `<div class="checking">Checking your membership…</div>`);
      let membership = "unknown";
      try {
      ...
      if (isLockedMembership(membership)) {
        this.set(this.css(CSS26) + `<div class="splash">
        <div class="lock">🔒</div>
      ...
      this.set(this.css(CSS26) + `<slot></slot>`);
    }
  };
...
`;
  var GbtiOnboarding = class extends GbtiElement {
    ...
    render() {
      ...
      if (!s) {
        this.set(this.css(CSS27) + `<p class="note">Checking your setup...</p>`);
        return;
      }
      ...
      if (s.ready) {
        ...
        this.set(this.css(CSS27) + `<div class="ready">${check(true)}<div class="big">You are ready to publish</div>
        <p class="note">${note}</p>
...
`;
  var GbtiTopicPicker = class extends GbtiElement {
    ...
    render() {
      if (!this._topics) {
        this.set(this.css(CSS28) + `<p class="muted">Loading topics...</p>`);
        return;
      }
      if (!this._topics.length) {
        this.set(this.css(CSS28) + `<p class="muted">No topics available right now.</p>`);
        return;
      }
...
`;
  var GbtiSaved = class extends GbtiElement {
    ...
    render() {
      if (!this.client) {
        this.set(this.css(CSS30) + `<p class="muted">Sign in with the GBTI client to manage your saved items.</p>`);
        return;
      }
      if (!this._activity) {
        this.set(this.css(CSS30) + `<p class="muted">Loading your saved items...</p>`);
        return;
      }
      if (this._activity.error === "not-authenticated") {
        this.set(this.css(CSS30) + `<p class="muted">Sign in to manage favorites and collections.</p>`);
        return;
      }
...
`;
  var GbtiSubscriptions = class extends GbtiElement {
    ...
    render() {
      if (!this.client) {
        this.set(this.css(CSS31) + `<p class="muted">Sign in with the GBTI client to manage who you follow.</p>`);
        return;
      }
      if (!this._loaded) {
        this.set(this.css(CSS31) + `<p class="muted">Loading your follows...</p>`);
        return;
      }
...
`;
  var GbtiWorkspace = class extends GbtiElement {
    ...
    render() {
      ...
      if (this._editing) {
        this.set(this.css(CSS32) + `<button class="btn back" data-back type="button">&larr; Back to my work</button><gbti-content-editor></gbti-content-editor>`);
        this.on("[data-back]", "click", () => {
          this._editing = null;
      ...
      if (this._reviewing != null) {
        this.set(this.css(CSS32) + `<button class="btn back" data-back type="button">&larr; Back to inbox</button><gbti-contrib-review number="${esc(this._reviewing)}"></gbti-contrib-review>`);
        this.on("[data-back]", "click", () => {
          this._reviewing = null;
          this.render();
        });
      ...
      this.set(this.css(CSS32) + `${this._profileHtml()}<div class="tabs" role="tablist">${tabs}</div><div data-body>${this._body()}</div>`);
      this.$$("[data-tab]").forEach((b) => b.addEventListener("click", () => {
        this._tab = b.dataset.tab;
        this._msg = null;
        this._draftMsg = null;
    ...
    _contentRow(it, i) {
      ...
      const view = pub ? `<a class="btn" href="${esc(isExt ? SITE11 + pub : pub)}"${isExt ? ' target="_blank" rel="noopener"' : ""} title="View the live page">View</a>` : "";
      return `<li class="row"><span class="gl" style="--ka:${esc(g.accent)}"><svg viewBox="0 0 24 24" aria-hidden="true">${g.svg}</svg></span><span class="t"><b>${esc(it.title)}</b><span class="meta">${esc(it.type || "")}</span></span><span class="right">${status2} ${stagedTag} ${vis}${view}<button class="btn" data-edit="${i}" type="button">Manage</button>${flip}</span></li>`;
    }
...
`;
  var GbtiActivityBell = class extends GbtiElement {
    ...
    async _prs() {
      ...
      return prs.filter((p) => p.merged === true || p.state === "merged" || p.state === "closed").map((p) => {
        ...
        return {
          ...
          href: lc12.needsAttention ? "workspace.html#tab=prs" : p.html_url || SITE12
        };
      });
    }
    async _following(login) {
      ...
      if (!set.size) return [];
      const res = await fetch(`${SITE12}/activity-index.json`, { cache: "no-cache" });
      ...
      return entries.filter((e) => set.has(String(e.author).toLowerCase())).map((e) => ({
        ...
        sub: `@${e.author}`,
        href: e.path ? `newtab.html#${buildReadHash(e.type, e.path)}` : `${SITE12}${e.url || ""}`
      }));
    }
    ...
    render() {
      ...
      this.set(this.css(CSS33) + `<button class="btn" type="button" data-bell aria-label="Activity${total ? `, ${total} new` : ""}" aria-haspopup="true" aria-expanded="${this._open}">${BELL}${dot}</button>${panel}`);
...
`;
  var GbtiNews = class extends GbtiElement {
    ...
    render() {
      ...
      this.set(this.css(CSS34) + head + `<div data-body></div>`);
      this.$$("[data-view]").forEach((b) => b.addEventListener("click", () => this._setView(b.dataset.view)));
...
`;
  var GbtiNewsReader = class extends GbtiElement {
    ...
    render() {
      if (!this.client) {
        this.set(this.css(CSS35) + `<p class="muted">Open in the GBTI client to read the news.</p>`);
        return;
      }
      const it = this._item;
      if (!it) {
        this.set(this.css(CSS35) + `<p class="muted">No item selected.</p>`);
        return;
      }
      ...
      this.set(this.css(CSS35) + `<div class="wrap"><div class="cols"><div class="main">` + hero + `<h2>${esc(it.title || "News")}</h2>` + (it.category ? `<div class="metarow"><span class="mlabel">Category</span><span class="catchip">${esc(it.category)}</span></div>` : "") + `<p class="sum">${esc(it.excerpt || "No summary available.")}</p><div class="acts">${open ? `<a class="src" href="${esc(open)}" target="_blank" rel="noopener noreferrer">Open source ↗</a>` : ""}${disc}</div>${note}</div><aside class="side">${chanCard}${discussion}</aside></div></div>`);
      if (!this._wiredErr) {
        this.root?.addEventListener("error", (e) => {
          const t = e.target;
...
`;
  var GbtiSyndicateNow = class extends GbtiElement {
    render() {
      ...
      this.set(this.css(CSS36) + `<button class="snbtn" type="button">Manually Syndicate</button>${this._open ? this._modalHtml() : ""}`);
      this.on(".snbtn", "click", () => {
        this._open = true;
        this._step = "dest";
        this._result = null;
...
`;
  var GbtiReader = class extends GbtiElement {
    ...
    async _resolveAuthor(it) {
      ...
      const me = lc9(status2?.identity?.username || status2?.identity?.login);
    ...
    render() {
      ...
      const view = it.type === "share" ? it.url ? `<a class="view" href="${esc(shareOut)}" target="_blank" rel="noopener nofollow">${embedUrl(it.url) ? "Watch video" : "Read article"} on ${esc(hostOf2(it.url))}</a>` : "" : it.url ? `<a class="view" href="${esc(SITE15 + it.url)}" target="_blank" rel="noopener">View on gbti.network</a>` : "";
...
`;
  var GbtiMemberView = class extends GbtiElement {
    _loaded = false;
    _loading = false;
    _entry = null;
    // /members-index.json row, or null (off-directory member)
    _isSelf = false;
    _sections = {};
    // type -> filtered items[]
    static get observedAttributes() {
    ...
    async _load() {
      ...
      try {
        const [dir, status2, ...idx] = await Promise.all([
        ...
        const me = lc11(status2?.identity?.username || status2?.identity?.login || "");
    ...
    _heroHtml() {
      ...
      if (e.joinedAt) {
        try {
          since = `<div class="since">Member since ${new Date(e.joinedAt).toLocaleDateString(void 0, { year: "numeric", month: "long" })}</div>`;
        } catch {
          since = "";
        }
      }
    ...
    render() {
      const username = this._username;
      if (!username) {
        this.set(this.css(CSS38) + `<div class="wrap"><div class="note">No member selected.</div></div>`);
        return;
      }
      if (this.client && !this._loaded && !this._loading) {
        this._loading = true;
      ...
      this.set(this.css(CSS38) + `<div class="wrap">${this._heroHtml()}${sections}</div>`);
      if (this._loaded) {
...
`;
  var GbtiBrowse = class extends GbtiElement {
    ...
    render() {
      if (this._reading) {
        ...
        this.set(this.css(CSS39) + `<button class="btn" data-back type="button">&larr; Back to ${esc(label)}</button><div data-reader></div>`);
        this.on("[data-back]", "click", () => {
          this._reading = null;
          this.render();
          this._ensureTab(this._tab);
      ...
      this.set(this.css(CSS39) + `<div class="tabs" role="tablist">${tabs}</div><div data-body></div>`);
      this.$$("[data-tab]").forEach((b) => b.addEventListener("click", () => {
        this._tab = b.dataset.tab;
        this._cat = [];
        this.render();
  ...
  async function refreshAccount() {
    ...
    if (id && s.authenticated) {
      const lapsed = LOCKED5.includes(s.membership) ? ` Your membership has lapsed. <a href="${SITE18}/membership/" target="_blank" rel="noopener">Renew</a> to publish again.` : "";
      ...
      el.innerHTML = `Not a member yet? <a href="${SITE18}/membership/" target="_blank" rel="noopener">Join GBTI Network</a> to publish.`;
    }
  }
  function mount() {
