...
Named deployments do exist, and they cluster in exactly the categories the design implies.

| Company | What the App Clip does | Invocation |
|---|---|---|
| ExxonMobil | Tap to pay at the pump, 2020[^4] | At the pump |
| Apple Store | Scan a barcode for accessory details, 2021[^4] | Barcode in store |
| Panera Bread | Order from a nearby location[^4] | Apple Maps |
| ParkWhiz | Pay for a parking spot without installing anything[^6] | NFC tag at the spot |
| SignEasy | Sign a document sent to you, no install for the signer[^6] | Shared link |
| Parcel | Track one package from a shared link[^6] | Shared link |
| Elloveo | Free trial of a children's science app[^6] | Demo |
| Flash Note Cards | Make and share cards inside a conversation[^6] | Messages |

Two patterns are visible in that list. Every one of them is a task that finishes in about a minute, and several of them put the clip in front of somebody who is not the customer at all. The person signing a SignEasy document, or opening a shared Parcel link, has no reason to want the app. That is arguably the strongest case for the whole feature, and it is not the retail case Apple leads with.
